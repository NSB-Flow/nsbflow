...
DECLARE
  ...
  v_incoming_ref text;
  v_referrer_id uuid;
  v_credits_amount int := 5;
  v_phone text;
BEGIN
  v_ref_code := upper(substr(replace(gen_random_uuid()::text,'-',''),1,8));
  v_phone := NULLIF(trim(NEW.raw_user_meta_data->>'phone_number'), '');

  INSERT INTO public.profiles (id, full_name, referral_code, phone_number)
  VALUES (NEW.id, COALESCE(NEW.raw_user_meta_data->>'full_name', NEW.email), v_ref_code, v_phone)
